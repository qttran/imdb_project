...
def Regression_Models(inputs, outputs_rating, outputs_revenue, preprocessed_features, 
	...
	clf_rating = linear_model.RidgeCV(alphas = [0.01, 0.1, 1.0, 10.0, 100.0, 1000.0], normalize = True)
	...
	clf_revenue = linear_model.RidgeCV(alphas = [1e+05, 1e+06, 1e+07, 1e+08, 1e+09])
	...
	#clf_rating = linear_model.LassoCV(alphas = [0.01, 0.1, 1.0, 10.0, 100.0], normalize = True)
	#clf_rating.fit(inputs, outputs_rating)
	#print 'The R-square of prediction is', clf_rating.score(inputs, outputs_rating)
	#print clf_rating.alpha_, clf_rating.coef_ 

	#clf_revenue = linear_model.LassoCV(alphas = [1e+03, 1e+04, 1e+05, 1e+06, 1e+07])
	#clf_revenue = clf_revenue.fit(inputs, outputs_revenue)
	#print 'The R-square of prediction is', clf_revenue.score(inputs, outputs_revenue)
	#print clf_revenue.alpha_, clf_revenue.coef_ 
	...
	#clf_revenue = linear_model.LinearRegression(normalize = True)
	#clf_revenue.fit(inputs, outputs_revenue)
	#print 'The R-square of predition is', clf_revenue.score(inputs, outputs_revenue)
	#print clf_revenue.coef_
	...
	random_forest_rating = ensemble.RandomForestRegressor(n_estimators = 10, 
		max_depth = 5, random_state = 0)
	random_forest_rating.fit_transform(inputs, outputs_rating)
	#print random_forest_rating.estimators_
	#print random_forest_rating.feature_importances_
	#print 'The 10-fold cross validations are', \
	#cross_val_score(random_forest_rating, inputs, outputs_rating, cv=10)

	random_forest_revenue = ensemble.RandomForestRegressor(n_estimators = 10, 
		max_depth = 5, random_state = 0)
	random_forest_revenue.fit_transform(inputs, outputs_revenue)
	#print random_forest_revenue.estimators_
	#print random_forest_revenue.feature_importances_
	#print 'The 10-fold cross validations are', \
	#cross_val_score(random_forest_revenue, inputs, outputs_rating, cv=10)
	pickle.dump(clf_rating, open("clf_rating.p", "wb"))
	pickle.dump(clf_revenue, open("clf_rating.p", "wb"))
	pickle.dump(random_forest_revenue, open("random_forest_revenue.p", "wb"))
	pickle.dump(random_forest_rating, open("random_forest_rating.p", "wb"))
	...
	for line in preprocessed_movies_top_actors:
		...
		#print 'The predicted rating is', clf_rating.predict(test_input)
		#print 'The predicted rating is', tree_rating.predict(test_input)
		#print 'The predicted rating is', random_forest_rating.predict(test_input)
		
		print 'The true revenue is', test_output_revenue
		#print 'The predicted revenue is', clf_revenue.predict(test_input)
		#print 'The predicted revenue is', tree_revenue.predict(test_input)
		print 'The predict rating is', random_forest_revenue.predict(test_input)
